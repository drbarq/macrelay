...
async fn contacts_get_all_returns_results() {
    let r = reg();
    let result = call_ok(&r, "contacts_get_all", json!({})).await;
    ...
    assert!(
        ...
        "contacts_get_all returned unexpected output: {}",
        text
    );
}
...
async fn contacts_search_runs_without_panic() {
    ...
    let result = r
        .call_tool("contacts_search", args(json!({ "query": "a" })))
        .await;
    ...
    match result {
        Ok(res) => {
            ...
            assert!(
                text.contains("| Emails:") || text.contains("No contacts found"),
                "contacts_search returned unexpected output: {}",
                text
            );
        }
